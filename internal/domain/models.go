...
type ModerationWorkspace struct {
	ID          string    `json:"id"`
	Name        string    `json:"name"`
	Description string    `json:"description,omitempty"`
	CreatedAt   time.Time `json:"created_at"`
}
type ModerationChannel struct {
	ID                    string    `json:"id"`
	ModerationWorkspaceID string    `json:"workspace_id"`
	Name                  string    `json:"name"`
	CreatedAt             time.Time `json:"created_at"`
}
...
type RuleBundle struct {
	ID                    string    `json:"id"`
	ModerationWorkspaceID string    `json:"workspace_id"`
	ModerationChannelID   string    `json:"channel_id"`
	Key                   string    `json:"key"`
	Description           string    `json:"description,omitempty"`
	Type                  ValueType `json:"type"`
	DefaultValue          any       `json:"default_value"`
	Rules                 []Rule    `json:"rules,omitempty"`
	ActiveBundleRevision  int       `json:"active_revision"`
...
type BundleRevision struct {
	...
	RuleBundleID string `json:"bundle_id"`
	Value        any    `json:"value"`
	Rules        []Rule `json:"rules,omitempty"`
	Status       string `json:"status"`
	CreatedAt    time.Time
...
func (f *RuleBundle) Validate() error {
	if f.ID == "" || f.ModerationWorkspaceID == "" || f.ModerationChannelID == "" || f.Key == "" {
		return fmt.Errorf("%w: bundle identity is required", ErrInvalid)
	}
...
func (v *BundleRevision) Validate(t ValueType) error {
	if v.Number < 1 {
