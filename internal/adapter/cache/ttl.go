...
type entry struct {
	revision domain.BundleRevision
	expires  time.Time
}
type TTL struct {
	mu   sync.RWMutex
...
func (c *TTL) Get(_ context.Context, key string) (*domain.BundleRevision, bool) {
	c.mu.RLock()
	e, ok := c.data[key]
	c.mu.RUnlock()
	...
	if e.revision.Status != "published" {
		c.Delete(context.Background(), key)
		return nil, false
	}
	v := e.revision
...
func (c *TTL) Set(_ context.Context, key string, v domain.BundleRevision) {
